 ...
 * 01 位标识，由于 long 在 Java 中是有符号的，最高位是符号位，正数是 0，负数是 1，ID 一般使用正数，所以最高位是 0
 * 39 位时间截(毫秒级)，注意，时间截不是存储当前时间的时间截，而是存储时间截的差值(当前时间 - 开始时间)得到的值，
 * 开始时间截，一般是业务开始的时间，由我们程序来指定，如 SnowflakeIdWorker 中的 START_TIMESTAMP 属性。
 * 1
 * 39 位的时间截，可以使用 17 年: (2^39)/(1000*60*60*24*365) = 17.43 年
 * 1
 * 5 位的数据机器位，可以部署在 32 个节点
 * 8 位序列，毫秒内的计数，计数顺序号支持每个节点每毫秒(同一机器，同一时间截)产生 256 个 ID 序号
 * 特性：
...
public class UuidUtil {
     ...
     */
    private UuidUtil() {
        this.machineId = PropertyUtil.getProperty("snowflakeMachineId", Long.class, -1L);
        AssertUtil.isFalse(machineId == -1, "同一个应用的不同运行容器需要配置不同的工作机器的序号，用于从根本上避免ID碰撞");
        AssertUtil.isTrue(machineId >= 0 && machineId <= 31, "machineId必须介于0-31之间");
    }
